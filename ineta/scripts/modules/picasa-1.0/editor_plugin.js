		...
		init : function(ed, url) {
			...
			ed.addCommand('mce'+pName, function() {
				ed.windowManager.open({
					file : './?adminURL&tinyMcePicasaAlbumRequest',
					width : 500,
					height : 150,
					inline : 1
				}, {
			...
			ed.getSelectedPluginPicasa = function(tagName){
				...
				if(match = matchTag.exec(content)){
					if(match[2]!=''){
						return this.parseStringToPluginPicasa(match[2]);
					}
				}
				return false;
			};
			...
			ed.parseStringToPluginPicasa = function(str){
			...
			ed.parsePluginPicasaToString = function(obj){
				if(typeof(obj)=="object"){
			...
			ed.getImageHtmlPicasa = function(alt){
				return '<img src="'+url+'/img/trans.gif" alt="'+alt+'" class="'+pName+' mceItem mceItemNoResize" title="'+ed.getLang('picasa.desc')+'" />';
			};
			
			/* Create the args for the item */
			ed.getImageArgsPicasa = function(alt){
				var args = {};
				tinymce.extend(args, {
			...
			ed.insertPluginPicasa = function(obj){
				var data = ed.parsePluginPicasaToString(obj);
				if(data!==false){
					var args = ed.getImageArgsPicasa(data), el;
		...
		_handle : function(ed, url) {
					...
					ed.theme.onResolveName.add(function(th, o) {
						if (o.node.nodeName == 'IMG') {
							if ( ed.dom.hasClass(o.node, pName) ){
								o.name = ed.getLang('picasa.title');
							}
						}
			...
			ed.onBeforeSetContent.add(function(ed, o) {
				...
				while (match = matchTag.exec(o.content)){
					_html = ed.getImageHtmlPicasa(((match[2])?match[2]:''));
