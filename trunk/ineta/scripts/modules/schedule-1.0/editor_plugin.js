	...
	tinymce.create(pFunctionName, {
		...
		init : function(ed, url) {
			...
			ed.addCommand('mce'+pName, function() {
				ed.insertPluginSchedules(new Object());
			});

			/* Register button */
			ed.addButton(pName, {
				title : ed.getLang('schedules.desc'),
				cmd : 'mce'+pName,
			...
			ed.getSelectedPluginSchedules = function(tagName){
				...
				if(match = matchTag.exec(content)){
					if(match[2]!=''){
						return this.parseStringToPluginSchedules(match[2]);
					}
				}
				return false;
			};
			...
			ed.parseStringToPluginSchedules = function(str){
			...
			ed.parsePluginSchedulesToString = function(obj){
				if(typeof(obj)=="object"){
			...
			ed.getImageHtmlSchedules = function(alt){
				return '<img src="'+url+'/img/trans.gif" alt="'+alt+'" class="'+pName+' mceItem mceItemNoResize" title="'+ed.getLang('schedules.desc')+'" />';
			};
			
			/* Create the args for the item */
			ed.getImageArgsSchedules = function(alt){
				var args = {};
				tinymce.extend(args, {
			...
			ed.insertPluginSchedules = function(obj){
				var data = ed.parsePluginSchedulesToString(obj);
				if(data!==false){
					var args = ed.getImageArgsSchedules(data), el;
		...
		_handle : function(ed, url) {
					...
					ed.theme.onResolveName.add(function(th, o) {
						if (o.node.nodeName == 'IMG') {
							if ( ed.dom.hasClass(o.node, pName) ){
								o.name = ed.getLang('schedules.title');
							}
						}
			...
			ed.onBeforeSetContent.add(function(ed, o) {
				...
				while (match = matchTag.exec(o.content)){
					_html = ed.getImageHtmlSchedules(((match[2])?match[2]:''));
