	...
	tinymce.create(pFunctionName, {
		...
		init : function(ed, url) {
			...
			ed.addCommand('mce'+pName, function() {
				ed.insertPluginSubscriptions(new Object());
			});

			/* Register button */
			ed.addButton(pName, {
				title : ed.getLang('subscriptions.desc'),
				cmd : 'mce'+pName,
			...
			ed.getSelectedPluginSubscriptions = function(tagName){
				...
				if(match = matchTag.exec(content)){
					if(match[2]!=''){
						return this.parseStringToPluginSubscriptions(match[2]);
					}
				}
				return false;
			};
			...
			ed.parseStringToPluginSubscriptions = function(str){
			...
			ed.parsePluginSubscriptionsToString = function(obj){
				if(typeof(obj)=="object"){
			...
			ed.getImageHtmlSubscriptions = function(alt){
				return '<img src="'+url+'/img/trans.gif" alt="'+alt+'" class="'+pName+' mceItem mceItemNoResize" title="'+ed.getLang('subscriptions.desc')+'" />';
			};
			
			/* Create the args for the item */
			ed.getImageArgsSubscriptions = function(alt){
				var args = {};
				tinymce.extend(args, {
			...
			ed.insertPluginSubscriptions = function(obj){
				var data = ed.parsePluginSubscriptionsToString(obj);
				if(data!==false){
					var args = ed.getImageArgsSubscriptions(data), el;
		...
		_handle : function(ed, url) {
					...
					ed.theme.onResolveName.add(function(th, o) {
						if (o.node.nodeName == 'IMG') {
							if ( ed.dom.hasClass(o.node, pName) ){
								o.name = ed.getLang('subscriptions.title');
							}
						}
			...
			ed.onBeforeSetContent.add(function(ed, o) {
				...
				while (match = matchTag.exec(o.content)){
					_html = ed.getImageHtmlSubscriptions(((match[2])?match[2]:''));
